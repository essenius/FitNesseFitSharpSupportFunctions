...
namespace SupportFunctions
{
    ...
    public class TestControl
    {
        ...
        {
            {string.Empty, "Control stopping tests/suites from within a test"},
            {nameof(StopSuiteIf), "Throw a StopSuiteException if argument is true"},
            {nameof(StopSuiteIfNot), "Throw a StopSuiteException if argument is false"},
            {nameof(StopTestIf), "Throw a StopTestException if argument is true"},
            {nameof(StopTestIfNot), "Throw a StopTestException if argument is false"}
        };

        public static void StopSuiteIf(string condition)
        {
            ThrowStopIf(condition, true, true);
        }

        public static void StopSuiteIfNot(string condition)
        {
            ThrowStopIf(condition, true, false);
        }

        public static void StopTestIf(string condition)
        {
            ThrowStopIf(condition, false, true);
        }

        public static void StopTestIfNot(string condition)
        {
            ThrowStopIf(condition, false, false);
        }
        ...
        private static void ThrowStopIf(string conditionString, bool stopSuite, bool expectedValue)
        {
            ...
            if (canParse && condition != expectedValue)
            {
                return;
            }
